...
int 
   ...
   BLOCK_THICKNESS = 14 , //(int)( BLOCK_WIDTH * 0.5 ) , 
...
int Freak::Game_Init(HWND hand)
   ...
   ball_dy = -BALL_Y_VELOCITY ; // move up to start
         ...
         if (blocks[row][col] != 0)
           {
            ...
            bb.Draw_Rectangle( x1-SHADOW_OFFSET, y1+SHADOW_OFFSET,
...
void Freak::Check_Keys(HWND main_window_handle)
   ...
   if( KEY_DOWN(VK_END) ) game_state = GAME_STATE_PAUSE ;

   // check if user is trying to resume
   else
		   if( KEY_DOWN(VK_HOME) ) game_state = GAME_STATE_RUN ;
