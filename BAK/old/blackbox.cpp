...
int DD_Init(int width, int height, int bpp)
   ...
   for (index=0; index < 256; index++)
     {
      // set each entry
		  if (index == 0)      // create black for shadows
				palette[index].peBlue = palette[index].peGreen = palette[index].peRed = 1 ;
      else if (index < 64) // brighter shades of red
        palette[index].peRed = 127 + index*2; 
      else                 // brighter shades of green
        if (index >= 64 && index < 128) 
          palette[index].peGreen = 127 + (index-64)*2;
        else               // brighter shades of blue
          if (index >= 128 && index < 192) 
            palette[index].peBlue = 127 + (index-128)*2;
					else if (index == 192)  // yellow for paddle
						{
						 palette[index].peRed = 227 ;
						 palette[index].peGreen = 227 ;
						}
						else 	         // shades of grey for background & ball
   ...
   if (lpdd->CreatePalette(DDPCAPS_8BIT | DDPCAPS_INITIALIZE 
		           | DDPCAPS_ALLOW256, palette,&lpddpal,NULL)!=DD_OK)
     return(0);
...
SetTextColor( xdc,RGB(palette[color].peRed,
