...
// screen parameter
int SCREEN_BPP    =   8 ; // maximum colors
...
Freak::Freak()
  {
   paddle_x = paddle_prev_x = 0 ; // track position of paddle

   ball_x = ball_y = 0;   // track position of ball
   ball_dx = ball_dy = 0; // velocity of ball

   score = 0;          // the score
   level = 1;          // the current level

   blocks_hit =  0 ;    // track number of blocks hit
   misses	    = -1 ;	  // number of times ball goes by paddle
     ...
     if( score <= LO_SCORE )
		   bb.Draw_Text_GDI(buffer, 8, TEXT_Y, LO_SCORE_COLOR) ;
		        ...
		        bb.Draw_Text_GDI(buffer, 8, TEXT_Y, NEG_SCORE_COLOR) ;
			         ...
			         bb.Draw_Text_GDI(buffer, 8, TEXT_Y, POS_SCORE_COLOR) ;
           else bb.Draw_Text_GDI(buffer, 8, TEXT_Y, HI_SCORE_COLOR) ;
...
void Freak::Move_Ball(void)
   ...
   if( ball_y < 0 ) 
     ...
     if( ball_y > (SCREEN_HEIGHT - BALL_SIZE) )
        ...
        if( misses > 0 ) score -= ((level*DEC_MULTIPLIER) + BASE_DECREMENT); 
