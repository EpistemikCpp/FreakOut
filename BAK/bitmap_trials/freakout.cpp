...
//#define THOMAS //*/ MARIE_LAURE
...
#elif defined( THOMAS )
    // CURRENT SETTINGS MAKE LGREEN ROW INCOLLIDABLE AT LEVEL 2
    #define GAME_STRING  "Thomas Rainbow!"
    int PADDLE_WIDTH      =  50 ,	   
        PADDLE_THICKNESS  =   7 ,		
    // ball parameters
        BALL_SIZE         =   9 ,	 
    ...
    #define GAME_STRING "RAINBOW FREAKOUT!" 
    int PADDLE_WIDTH      =  48 ,	  
        PADDLE_THICKNESS  =   8 ,		
    // ball parameters
...
int BACKGROUND_COLOR    = GREY , 
    ...
    NEG_SCORE_COLOR     = RED ,  
    HI_SCORE_COLOR      = ORANGE ,  
    LO_SCORE_COLOR      = BLACK ,  
...
void Freak::Game_Init( HWND hand )
{
...
void Freak::Game_Main( HWND main_window_handle, int mouse_x )
{
   ...
   msg, score, level, ((level-1)*NUM_BLOCKS + blocks_hit), misses, high_streak);
 
	  // change color depending on the score
     if( score <= LO_SCORE )
		 bb.GDI_Draw_Text( buffer, 8, TEXT_Y, LO_SCORE_COLOR );
		else if( score < 0 )
		    bb.GDI_Draw_Text( buffer, 8, TEXT_Y, NEG_SCORE_COLOR );
        else if( score > SUPER_SCORE )
		      bb.GDI_Draw_Text( buffer, 8, TEXT_Y, SUPER_SCORE_COLOR );
          else if( score > HI_SCORE )
			     bb.GDI_Draw_Text( buffer, 8, TEXT_Y, HI_SCORE_COLOR );
            else
                bb.GDI_Draw_Text( buffer, 8, TEXT_Y, POS_SCORE_COLOR );
   ...
   else if( game_state == GAME_STATE_PAUSE )
   {
     ...
     bb.GDI_Draw_Text( buffer, 8, TEXT_Y, MAGENTA );
...
void Freak::Draw_Ball()     
{
   ...
   if( ball_dx > MAX_BALL_DX )
	   ball_dx = MAX_BALL_DX ;
	    ...
	    if( ball_dx < -MAX_BALL_DX ) ball_dx = -MAX_BALL_DX ;    

   // ORIGINAL THAT WORKS
   bb.DD_Blt_Rect( ball_x, ball_y, ball_x+BALL_SIZE, 
                   ball_y+BALL_SIZE, BALL_COLOR      );
/*
   // TRY TO MAP THE HAPPY_BITMAP AS THE BALL

 DDSURFACEDESC2 ddsdback ;
 lpddsback->GetSurfaceDesc( &ddsdback );

 // lock the surface
// lpddsback->Lock( NULL, &ddsdback, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT, NULL );

 UCHAR *video_buffer = (UCHAR*)ddsdback.lpSurface ;  // pointer to video buffer surface
// int mempitch = ddsdback.lPitch ;         // video pitch per line

// WRITE THE FILE INFO OUT 
if( FILE *bmInfo = fopen( "happy_bitmap_info.txt", "a+" ) )
{
  time_t ltime ;
  time( &ltime );
  fprintf( bmInfo, "\nCurrent time is %s", ctime(&ltime) );

  fprintf( bmInfo, "&video_buffer == %p \n", video_buffer );
  fprintf( bmInfo, "video_buffer lPitch == %p \n", ddsdback.lPitch );

  fprintf( bmInfo, "&happy_bitmap == %p \n", happy_bitmap );

//  for( int i=0; i < 64; i++ )
//    fprintf( bmInfo, "happy_bitmap[%u] == %d \n", happy_bitmap[i] );

  fclose( bmInfo );
}
 // THESE DON'T WORK
//   bb.DD_bltBitmap( ball_x, ball_y, 8, 8, happy_bitmap,
//                    video_buffer, ddsdback.lPitch       ); // happy_bitmap, 

//   bb.DD_bltHappy( ball_x, ball_y, ball_x+BALL_SIZE, 
//                   ball_y+BALL_SIZE, BALL_COLOR );
...
void Freak::Check_Keys( HWND main_window_handle )
{
   ...
   // check if user is trying to exit
   else if( KEY_DOWN(VK_ESCAPE) )
     {
       ...
       /*MHS: send a Destroy msg to WindowProc and it will post the Quit
              message we need to exit the main game loop in WinMain()    */
